...
//action
import { getHistory } from "../../Store/History/history.action";
...
const UserHistoryTable = (props) => {
  ...
  const user = JSON.parse(localStorage.getItem("HistoryUser"));
  ...
  useEffect(() => {
    props.getHistory(page, rowsPerPage, type, sDate, eDate, user._id); // eslint-disable-next-line
  }, [page, rowsPerPage, sDate, eDate, type, user._id]);
  ...
  const getAllHistory = () => {
    ...
    seteDate("ALL");
    $("#datePicker").removeClass("show");
    props.getHistory(page, rowsPerPage, type, sDate, eDate, user._id);
  };
                                  ...
                                  <tr>
                                    ...
                                    >
                                      {value?.type === 0
                                        ? value?.diamond !== 0
                                          ? value?.reelId
                                            ? `${value?.userName} send gift on your reel`
                                            : `${value?.userName} send gift to you`
                                          : value?.reelId
                                          ? `you send gift on ${value?.userName} reel`
                                          : `you send gift for ${value?.userName}`
                                        : value?.type === 1
                                        ? value?.isIncome
                                          ? `you get coin from convert diamond `
                                          : `you gave diamond for convert coin`
                                        : value?.type === 2
                                        ? `you purchase coin`
                                        : `you get coin for watching ad`}
                                    </td>
                                    <td
                                      className={
                                        value?.type === 0
                                    ...
                                    <td
                                      className={
                                        value?.type === 0
                                          ? value?.coin !== 0 && "text-danger"
                                          : value?.type !== 0
                                          ? value?.coin !== 0 && "text-success"
                                          : ""
                                      }
                                    >
                                      {" "}
