...
// type
import { OPEN_DIAMOND_PLAN_DIALOG } from "../../Store/diamondPlan/types";
...
import DiamondPlanDialog from "../Dialog/DiamondPlanDialog";
...
const DiamondPlan = (props) => {
  ...
  useEffect(() => {
    props.getDiamondPlan(); // eslint-disable-next-line
  }, []);

  const diamondPlan = useSelector((state) => state.diamondPlan.diamondPlan);
  ...
  const handleSearch = (e) => {
    ...
    if (value) {
      const data = diamondPlan.filter((data) => {
        return (
      ...
      return setData(diamondPlan);
    }
  };

  const handleOpen = () => {
    dispatch({ type: OPEN_DIAMOND_PLAN_DIALOG });
  };
    ...
    data
      .then((isDeleted) => {
        if (isDeleted) {
          ...
          props.deleteDiamondPlan(planId);
  ...
  const handleEdit = (data) => {
    dispatch({ type: OPEN_DIAMOND_PLAN_DIALOG, payload: data });
  };
            ...
            <div class="col-xl-6 col-md-6 col-sm-12 col-12 ">
              <div class="breadcrumb-four float-right">
                <ul class="breadcrumb">
                  ...
                  <li class="active">
                    <a href="javscript:void(0);"> Diamond Plan </a>
                  </li>
                </ul>
              </div>
            </div>
...
export default connect(null, { getDiamondPlan, deleteDiamondPlan })(
  DiamondPlan
);
